...
import { create } from "./api"; // endpoint create permission kamu
...
export default function PermissionForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await create({ name });
      showAlert("success", `Permission "${name}" berhasil dibuat ✅`);
      setName("");
    } catch (error: any) {
      showAlert("error", error.message || "Gagal membuat permission");
    } finally {
      setIsLoading(false);
    }
  };
